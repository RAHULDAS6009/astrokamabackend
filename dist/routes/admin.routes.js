...
router.get("/by-branch-code/:branchCode", async (req, res) => {
    try {
...
router.put("/bulk-roll-update-by-branch-code", async (req, res) => {
    try {
        ...
        /* ---------------- TRANSACTION UPDATE ---------------- */
        await prisma.$transaction(rolls.map((r) => prisma.student.update({
            where: {
                id: r.studentId,
            },
            data: {
